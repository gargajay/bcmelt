...
class AdminController extends Controller
{
    ...
    {
        Excel::import(new QuestionImport, $request->file('file')->store('temp'));
        return back();
    }
    ...
    public function manage_students($id){
        ...
        $data['exams']=Oex_exam_master::where('status','1')->whereNotIn('id',$ids)->get()->toArray();
        $data['students'] = user_exam::select(['user_exams.*','users.name','oex_exam_masters.title as ex_name','oex_exam_masters.exam_date'])
            ->join('users','users.id','=','user_exams.user_id')
            ->join('oex_exam_masters','user_exams.exam_id','=','oex_exam_masters.id')->where('user_exams.user_id',$id)->orderBy('user_exams.exam_id','desc')
            ->get()->toArray();
        $data['student_id'] =   $id;  
       ...
       // dd(1);
        ...
        if($validator->fails()){
            ...
            if(!empty($request->exam_id)){
                foreach($request->exam_id as $eid){

                    $std = new user_exam();
                    $std->exam_id = $eid;
                    $std->user_id = $request->student_id;
                    $std->std_status = 1;
                    $std->exam_joined = 0;
    ...
    public function add_new_students(Request $request){
        ...
        if($validator->fails()){
            ...
            $std->email = $request->mobile_no."@gmail.com";
            $std->mobile_no = $request->mobile_no;
            $std->exam = $request->password;
            $std->password = Hash::make($request->password);

            if($request->license_no){
                $std->license_no = $request->license_no;

            }
             if($request->comment){
                $std->comment = $request->comment;
    ...
    //Deleting students
    public function delete_students($id,$stu_id){

        $std = user_exam::where('id',$id)->get()->first();
        $std->delete();
        return redirect('admin/manage_students_exam/'.$stu_id);
    }


    public function edit_students(Request $request){

        $students = User::where('id',$request->id)->first();
    ...
    public function edit_students_final(Request $request){
        ...
        $std = User::where('id',$request->id)->first();
        $std->name = $request->name;
        $std->comment = $request->comment;
    ...
    public function registered_students(){
        ...
        $data['users'] = User::latest()->get()->all();
    ...
    public function add_questions(){
        ...
        $data['questions']=Oex_question_master::latest()->get()->toArray();
    ...
    public function add_new_question(Request $request){
        ...
        if($validator->fails()){
            ...
            $q->status=1;
            $q->eoption1 = $request->option_1;
            $q->eoption2 = $request->option_2;
            $q->eoption3 = $request->option_3;
            $q->eoption4 = $request->option_4;


            if(!empty($request->image_file))
            {
                $fileName = 'image_' . time() . '.'. $request->image_file->extension();  
            ...
            {
                $fileName = 'audio_' . time() . '.'. $request->audio_file->extension();  
                $request->audio_file->move(public_path('uploads'), $fileName);
                $q->audio_file = $fileName;
            }
...
//$arr = array('status'=>'true','message'=>'successfully added','reload'=>url('admin/add_questions'));
        }

        return  redirect('admin/add_questions')->with('success','successfully added');
    ...
    public function admin_view_result($id){
        ...
        $data['result_info'] = Oex_result::where('exam_id',$std_exam->exam_id)->where('user_id',$std_exam->user_id)->latest()->get()->first();
